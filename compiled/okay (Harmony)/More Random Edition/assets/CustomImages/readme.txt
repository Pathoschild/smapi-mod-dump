...
https://www.nexusmods.com/stardewvalley/mods/16921/ (for the fix and critter)
...
Critter
-------
Images here are replacement made in the critters.png spritesheet. It consists of most of the background non-interactable animals in Stardew.

DO NOT delete the critters.png image. It's used as the base image for all the replacements.

You may only add to these directories in groups. For a sprite to work, it needs to be a part of a group of sprites with the same name and different number, all amounting to the number of sprites the given critter type has.

The sprites in these directories are divided into three main categories. Big critter have 32x32px sprites. Small critter have 16x16px sprites. And there's unusually sized critters, that I have not yet implemented randomisation for.

Big Critter:
- Each sprite in the subdirectories for these critter have to have 32x32px. Each sprite group you make has to have exactly as many sprites as the base has, each ending in "_" and a number, designating which sprite it is, in order. E.g. "Ominous Seagull_3.png", for the third of fourteen sprites, of the Ominous Seagull group.
The Critter belonging in this category are:
	Seagull:  With 14 sprites.
	Crow:     With 11 sprites.
	Bird:     With 9 sprites.
	Rabbit:   With 7 sprites.
	Squirrel: With 8 sprites.
	Owl:      With 4 sprites.
	Gorilla:  With 7 sprites.

Small Critter:
- Each sprite in the subdirectories for these critter have to have 16x16px. Each sprite group you make has to have exactly as many sprites as the base has, each ending in "_" and a number, designating which sprite it is, in order. E.g. "Volatile Butterfly_2.png", for the second of four sprites, of the Volatile Butterfly group.
The Critter belonging in this category are:
	Butterfly:       With 4 sprites.
	Small Butterfly: With 3 sprites.
	Frog:		 With 7 sprites.
	Woodpecker:	 With 5 sprites.

Miscellaneous Critter:
- Each sprite in these subdirectories has different sizes. Each sprite group has to have exactly as many sprites as the base has, each ending in "_" and a number, designating which sprite it is, in order. E.g. "Orange_5.png", for the fifth of seven sprites, of the Orange Lava Monkey group.
The Critter belonging in this category are:
	Crab:		      With 6 18x18px sprites.
	Lava Monkey:	      With 7 20x25px sprites.
	Island Scream Animal: With 4 20x25px sprites.
	Lava Monkey Head:     With 3 14x12px sprites.
	Island Brown Bird:    With 5 12x12px sprites.