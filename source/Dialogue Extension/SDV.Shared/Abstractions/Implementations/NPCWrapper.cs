...
namespace SDV.Shared.Abstractions
{
  ...
  {
    public NPCWrapper(Character npc) : this(npc, null) { }

    public NPCWrapper(Character npc, IMonitor monitor) : base(npc)
    {
      if (monitor != null) monitor.Log("This is the npc : " + npc.Name);
    }

    public IDictionary<int, ISchedulePathDescriptionWrapper> Schedule { get; set; }
    public IDictionary<string, string> Dialogue { get; }
