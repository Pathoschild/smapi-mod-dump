...
namespace BetterShippingBox.Menus
{
    public class BetterShippingMenu : IClickableMenu
    {
        ...
        public BetterShippingMenu() : base(Game1.viewport.Width / 2 - (800 + IClickableMenu.borderWidth * 2) / 2,
            Game1.viewport.Height / 2 - (600 + IClickableMenu.borderWidth * 2) / 2,
            800 + IClickableMenu.borderWidth * 2, 600 + IClickableMenu.borderWidth * 2, true)
        {
            ...
            this.inventory = new InventoryMenu(
                this.xPositionOnScreen + Game1.tileSize / 2 + (Game1.tileSize / 3 + Game1.tileSize / 11),
                this.yPositionOnScreen + spaceToClearTopBorder + IClickableMenu.borderWidth + Game1.tileSize * 5 +
                Game1.pixelZoom * 10, false, null, new InventoryMenu.highlightThisItem(this.HighlightItemToShip))
            {
                showGrayedOutSlots = true
            };
            this.upArrow = new ClickableTextureComponent(
                new Rectangle(this.xPositionOnScreen + this.width + Game1.tileSize / 4,
                    this.yPositionOnScreen + Game1.tileSize / 4, 11 * Game1.pixelZoom, 12 * Game1.pixelZoom),
            ...
            this.scrollBarRunner = new Rectangle(this.scrollBar.bounds.X,
                ...
                this.height - Game1.tileSize - this.upArrow.bounds.Height - Game1.pixelZoom * 7);
            this.shippingBox =
                new ClickableComponent(
                    new Rectangle(this.xPositionOnScreen, this.yPositionOnScreen, this.width,
                        this.height - Game1.tileSize * 4 + Game1.tileSize / 2 + Game1.pixelZoom), (Item) null);
            for (int i = 0; i < 4; i++)
            {
                this.shippedItemButtons.Add(new ClickableComponent(
                    new Rectangle(this.xPositionOnScreen + Game1.tileSize / 4,
                        this.yPositionOnScreen + Game1.tileSize / 4 + i * ((this.height - Game1.tileSize * 4) / 4),
                        this.width - Game1.tileSize / 2, (this.height - Game1.tileSize * 4) / 4 + Game1.pixelZoom),
                    string.Concat(i)));
            }
        }
        ...
        public override void leftClickHeld(int x, int y)
        {
            ...
            if (!flag)
            {
                ...
                this.scrollBar.bounds.Y =
                    Math.Min(
                        this.yPositionOnScreen + this.height - Game1.tileSize - Game1.pixelZoom * 3 -
                        this.scrollBar.bounds.Height,
                        Math.Max(y, this.yPositionOnScreen + this.upArrow.bounds.Height + Game1.pixelZoom * 5));
                this.currentItemIndex = Math.Min(Game1.getFarm().shippingBin.Count - 4,
                    Math.Max(0,
                        (int) (Game1.getFarm().shippingBin.Count *
                               (double) ((y - this.scrollBarRunner.Y) / (float) this.scrollBarRunner.Height))));
        ...
        private void SetScrollBarToCurrentIndex()
        {
            ...
            {
                this.scrollBar.bounds.Y =
                    this.scrollBarRunner.Height / Math.Max(1, Game1.getFarm().shippingBin.Count - 4 + 1) *
        ...
        public override void receiveScrollWheelAction(int direction)
        {
            ...
            else
            {
                bool flag2 = direction >= 0 ||
                             this.currentItemIndex >= Math.Max(0, Game1.getFarm().shippingBin.Count - 4);
                if (!flag2)
                {
        ...
        public override void receiveLeftClick(int x, int y, bool playSound = true)
        {
            ...
            if (!flag)
            {
                ...
                bool flag3 = this.downArrow.containsPoint(x, y) &&
                             this.currentItemIndex < Math.Max(0, Game1.getFarm().shippingBin.Count - 4);
                if (flag3)
                {
                    this.DownArrowPressed();
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            bool flag6 = !this.downArrow.containsPoint(x, y) &&
                ...
                this.currentItemIndex =
                    Math.Max(0, Math.Min(Game1.getFarm().shippingBin.Count - 4, this.currentItemIndex));
                ...
                if (flag9)
                {
                    ...
                    if (flag10)
                    {
                        ...
                        if (flag11)
                        {
                            ...
                            {
                                this.animations.Add(
                                    new TemporaryAnimatedSprite(5, value + new Vector2(32f, 32f), Color.White)
                                    {
                ...
                {
                    bool flag13 = this.currentItemIndex + i >= Game1.getFarm().shippingBin.Count ||
                                  !this.shippedItemButtons[i].containsPoint(x, y);
                    if (!flag13)
                    {
                        int index = this.currentItemIndex + i;
                        bool flag14 = Game1.getFarm().shippingBin[index] != null &&
                                      this.TryToRetrieveItem(Game1.getFarm().shippingBin[index]);
                        if (flag14)
                        {
                        ...
                        this.currentItemIndex = Math.Max(0,
                            Math.Min(Game1.getFarm().shippingBin.Count - 4, this.currentItemIndex));
                        return;
                    }
                }

                bool flag15 = !this.readyToClose() || (x >= this.xPositionOnScreen - Game1.tileSize &&
                                                       y >= this.yPositionOnScreen - Game1.tileSize &&
                                                       x <= this.xPositionOnScreen + this.width + Game1.tileSize * 2 &&
                                                       y <= this.yPositionOnScreen + this.height + Game1.tileSize);
                if (!flag15)
                {
                    base.exitThisMenu();
                }
        ...
        private bool TryToRetrieveItem(Item item)
        {
            ...
            else
            {
                ...
                {
                    bool flag4 = !this.heldItem.Name.Equals(item.Name) ||
                                 ((SObject) this.heldItem).Quality != ((SObject) item).Quality ||
                                 this.heldItem.getStack() + item.getStack() >= this.heldItem.maximumStackSize();
                    if (flag4)
                    {
                        result = false;
                    }
        ...
        public override void receiveRightClick(int x, int y, bool playSound = true)
        {
            ...
            for (int i = 0; i < this.shippedItemButtons.Count; i++)
            {
                bool flag3 = this.currentItemIndex + i >= Game1.getFarm().shippingBin.Count ||
                             !this.shippedItemButtons[i].containsPoint(x, y);
                if (!flag3)
                {
                    int index = this.currentItemIndex + i;
                    bool flag4 = Game1.getFarm().shippingBin[index] != null &&
                                 this.TryToRetrieveItem(Game1.getFarm().shippingBin[index]);
                    if (flag4)
                    {
                        Game1.getFarm().shippingBin.RemoveAt(index);
        ...
        public override void performHoverAction(int x, int y)
        {
            ...
            if (!flag)
            {
                for (int i = 0; i < this.shippedItemButtons.Count; i++)
                {
                    bool flag2 = this.currentItemIndex + i < Game1.getFarm().shippingBin.Count &&
                                 this.shippedItemButtons[i].containsPoint(x, y);
                    if (flag2)
                    {
                ...
                if (!flag3)
                {
                    foreach (ClickableComponent current in this.inventory.inventory)
                    {
                        ...
                        if (flag4)
                        {
                            ...
                            bool flag5 = itemFromClickableComponent != null &&
                                         this.HighlightItemToShip(itemFromClickableComponent);
                            if (flag5)
                            {
                                this.hoverText = itemFromClickableComponent.Name + " x " +
                                                 itemFromClickableComponent.getStack();
                            }
                        }
                    }
                }
        ...
        public override void draw(SpriteBatch b)
        {
            ...
            IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(384, 373, 18, 18),
                this.xPositionOnScreen + Game1.tileSize / 2,
                this.yPositionOnScreen + this.height - Game1.tileSize * 4 + Game1.pixelZoom * 10,
                this.inventory.width + Game1.pixelZoom * 14, this.height - Game1.tileSize * 7 + Game1.pixelZoom * 5,
                Color.White, Game1.pixelZoom);
            IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(384, 373, 18, 18),
                this.xPositionOnScreen, this.yPositionOnScreen, this.width,
                this.height - Game1.tileSize * 4 + Game1.tileSize / 2 + Game1.pixelZoom, Color.White, Game1.pixelZoom);
            for (int i = 0; i < this.shippedItemButtons.Count; i++)
            {
                bool flag2 = this.currentItemIndex + i >= Game1.getFarm().shippingBin.Count ||
                             !Game1.getFarm().shippingBin.Any<Item>();
                if (!flag2)
                {
                    IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(384, 396, 15, 15),
                        this.shippedItemButtons[i].bounds.X, this.shippedItemButtons[i].bounds.Y,
                        this.shippedItemButtons[i].bounds.Width, this.shippedItemButtons[i].bounds.Height,
                        (!this.shippedItemButtons[i].containsPoint(Game1.getOldMouseX(), Game1.getOldMouseY()) ||
                         this.scrolling)
                            ? Color.White
                            : Color.Wheat, Game1.pixelZoom, false);
                    b.Draw(Game1.mouseCursors,
                        new Vector2(this.shippedItemButtons[i].bounds.X + Game1.tileSize / 2 - Game1.pixelZoom * 3,
                            this.shippedItemButtons[i].bounds.Y + Game1.pixelZoom * 6 - Game1.pixelZoom),
                        new Rectangle?(new Rectangle(296, 363, 18, 18)), Color.White, 0f, Vector2.Zero, Game1.pixelZoom,
                        SpriteEffects.None, 1f);
                    Game1.getFarm().shippingBin[this.currentItemIndex + i].drawInMenu(b,
                        new Vector2(this.shippedItemButtons[i].bounds.X + Game1.tileSize / 2 - Game1.pixelZoom * 2,
                            this.shippedItemButtons[i].bounds.Y + Game1.pixelZoom * 6), 1f);
                    SpriteText.drawString(b, Game1.getFarm().shippingBin[this.currentItemIndex + i].Name,
                        this.shippedItemButtons[i].bounds.X + Game1.tileSize * 3 / 2 + Game1.pixelZoom * 2,
                        this.shippedItemButtons[i].bounds.Y + Game1.pixelZoom * 7);
                }
            }
            ...
            if (flag3)
            {
                SpriteText.drawString(b, "Empty",
                    this.xPositionOnScreen + this.width / 2 - SpriteText.getWidthOfString("Empty") / 2,
                    this.yPositionOnScreen + this.height / 2 - Game1.tileSize * 2);
            }
            ...
            if (flag5)
            {
                IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(403, 383, 6, 6),
                    this.scrollBarRunner.X, this.scrollBarRunner.Y, this.scrollBarRunner.Width,
                    this.scrollBarRunner.Height, Color.White, Game1.pixelZoom);
            ...
            if (flag6)
            {
                IClickableMenu.drawToolTip(b, this.hoverText, this.boldTitleText, this.hoveredItem,
                    this.heldItem != null);
            }
