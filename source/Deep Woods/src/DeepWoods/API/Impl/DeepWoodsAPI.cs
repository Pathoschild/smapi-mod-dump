...
namespace DeepWoodsMod.API.Impl
{
    public class DeepWoodsAPI : IDeepWoodsAPI
    {
        ...
        public event Action<IDeepWoodsLocation> BeforeInfest;
        public event Action<IDeepWoodsLocation> AfterInfest;
        ...
        public event Func<IDeepWoodsLocation, bool> OverrideInfest;
        ...
        public void CallBeforeInfest(DeepWoods deepWoods)
        {
            if (BeforeInfest == null)
                return;

            foreach (Action<IDeepWoodsLocation> callback in BeforeInfest.GetInvocationList())
            {
                try
                {
                    callback(deepWoods);
        ...
        public void CallAfterInfest(DeepWoods deepWoods)
        {
            if (AfterInfest == null)
                return;

            foreach (Action<IDeepWoodsLocation> callback in AfterInfest.GetInvocationList())
            {
                try
                {
                    callback(deepWoods);
        ...
        public bool CallOverrideInfest(DeepWoods deepWoods)
        {
            if (OverrideInfest == null)
                return false;

            // If multiple mods add an override, we shuffle the overrides and the first one "wins":
            foreach (Func<IDeepWoodsLocation, bool> callback in ToShuffledList(OverrideInfest.GetInvocationList()))
            {
                try
                {
                    if (callback(deepWoods))
