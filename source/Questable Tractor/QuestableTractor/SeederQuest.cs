...
namespace NermNermNerm.Stardew.QuestableTractor
{
        ...
        : TractorPartQuest<SeederQuestState>
    {
        ...
        public SeederQuest(SeederQuestController controller)
            : base(controller)
        {
            this.questTitle = L("Fix the seeder");
            this.questDescription = L("Turns out George had the seeder attachment, maybe he can be talked into fixing it.");
        }
        ...
        public override void CheckIfComplete(NPC n, Item? item)
        {
            ...
            {
                this.Spout(n, L("Young people just don't listen.  I told you, I'm too old to repair your equipment.$a"));
            ...
            {
                this.Spout(n, L("Ohhh...  I know George *could* repair that thing...  But he could also fail miserably, you see, it's not just his legs that have let him down.  Have you seen his hands, how they shake?  It's not so bad, but it really frustrates him.#$b#Oh deary, I'm old too...  I need time to think about this."));
            ...
            {
                this.Spout(n, L("Sorry, what did you say?  I didn't quite hear..."));
            }
            ...
            {
                this.Spout(n, L("Look, I got my life to live, and it doesn't involve fixing farm equipment."));
            }
            ...
            {
                this.Spout(n, L("Nah man, these hands were made for gridball.  You're good with this sort of thing, you should talk Grandpa into showing you how.  Or get Maru, she's good at mechanical stuff.$3"));
            ...
            {
                this.Spout(n, L("Heh, isn't easy is it.  Keep it up, maybe you'll learn how to be mayor someday!$1#$b#Evelyn was probably right, in that somebody his own age could convince him.  Somebody he spends a lot of time with."));
            }
            else if ((n.Name == "Sebastian" || n.Name == "Abigail" || n.Name == "Sam") && isHoldingSeeder && this.State == SeederQuestState.GetHaleyOnSide)
            {
                this.Spout(n, L("You think *I* have a clue what goes on inside Alex's head??$5#$b#Oh wait, I *DO* know...  Absolutely nothing.$4"));
            }
            else if (n.Name == "Emily" && isHoldingSeeder && this.State == SeederQuestState.GetHaleyOnSide)
            {
                this.Spout(n, L("Oh I hate to admit it, but the solution to your problem is Haley.  She can make any boy do any thing, ESPECIALLY Alex.$2#$b#She might seem vaccuous at times, but trust me, if you need some guy manipulated, she can do it.  I'm not saying it's a good thing all the time, but this sounds like a good cause.$4"));
            }
            else if (n.Name == "Haley" && this.State == SeederQuestState.GetHaleyOnSide)
            {
                this.Spout(n, L("Hah.  Alex does more with his hands than just play Gridball.$3#$b#Tell me everything. every. little. detail.$7#$b#Ah...  I see...#$b#I'll take care of this for you.  Just give me a day or two.  I'll let you know.$2#$b#*Sigh*  Confidence is so very attractive in a man.$7"));
            ...
            {
                this.Spout(n, LF($"Okay, I can do this.  Grandpa seems fired up too.  Give me the {ironBarCount} iron bars Grandpa said we need and the broken seeder and we'll get on it."));
            ...
            else if (n.Name == "Alex" && this.State == SeederQuestState.GiveAlexStuff)
            {
                if (this.TryTakeItemsFromPlayer("(O)335", ironBarCount, ObjectIds.BustedSeeder, 1))
                {
                    ...
                    this.Spout(n, L("Thanks, that's all the stuff.  Well, I'm off the the garage with Gramps.  I'll send mail or something after we get it working."));
                }
                else if (isHoldingSeeder)
                {
                    this.Spout(n, LF($"We'll need the old seeder and {ironBarCount} iron bars.  Bring 'em by when you can."));
                }
            }
            else if (n.Name == "George" && this.State == SeederQuestState.GetPartFromGeorge)
            {
                this.AddQuestItemToInventory(ObjectIds.WorkingSeeder);
                this.Spout(n, L("There you go.  Fixed it myself.  Alex didn't screw it up too much; he's a good kid.#$b#Don't try and sprinkle chicken manure with the thing.  I don't want to see this thing back here again."));
        ...
        protected override void SetObjective()
        {
            switch (this.State)
            {
                ...
                case SeederQuestState.GetEvelynOnSide:
                    this.currentObjective = LF($"Get Evelyn to help (Talk to her after getting her to {evelynWillingToHelpLevel} hearts)");
                    break;
                case SeederQuestState.WaitForEvelyn:
                    this.currentObjective = L("Granny said she'll think about it.  I guess we'll wait to hear from her.");
                    break;
                case SeederQuestState.TalkToAlex1:
                    this.currentObjective = L("Granny's mail said I should gain Alex's trust (2 hearts) and try and talk him into it.");
                    break;
                case SeederQuestState.GetHaleyOnSide:
                    this.currentObjective = L("Alex still seems resistant to the idea.  Maybe I need to get somebody else to give me another angle.");
                    break;
                case SeederQuestState.WaitForHaleyDay1:
                    this.currentObjective = L("Wait for Haley to cajole Alex into helping.");
                    break;
                case SeederQuestState.TalkToAlex2:
                    this.currentObjective = L("Talk to Alex");
                    break;
                case SeederQuestState.GiveAlexStuff:
                    this.currentObjective = LF($"Bring Alex the old seeder and {ironBarCount} iron bars");
                    break;
                case SeederQuestState.WaitForAlexDay1:
                case SeederQuestState.WaitForAlexDay2:
                    this.currentObjective = L("Hopefully Alex and George are getting on with it.  Alex said he'd send mail when it's working.");
                    break;
                case SeederQuestState.GetPartFromGeorge:
                    this.currentObjective = L("Alex's mail says the seeder is done and I just need to get it from George.");
                    break;
                case SeederQuestState.InstallPart:
                    this.currentObjective = L("Bring the fixed seeder to the tractor garage.");
                    break;
            }
        }
