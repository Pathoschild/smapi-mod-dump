...
namespace NermNermNerm.Stardew.QuestableTractor
{
        ...
        : TractorPartQuest<LoaderQuestState>
    {
        ...
        public LoaderQuest(LoaderQuestController controller)
            : base(controller)
        {
            this.questTitle = L("Fix the loader");
            this.questDescription = L("I found the front end loader attachment for the tractor, but it's all bent up and rusted through in spots.");
        }
        ...
        protected override void SetObjective()
        {
            switch (this.State)
            {
                case LoaderQuestState.TalkToClint:
                    this.currentObjective = L("This looks like a the sort of thing a blacksmith could fix.");
                    break;
                case LoaderQuestState.FindSomeShoes:
                    this.currentObjective = L("Somehow I've been railroaded into finding Clint some new shoes.  Maybe some of the younger townspeople know where I could get some for cheap.");
                    break;
                case LoaderQuestState.SnagAlexsOldShoes:
                    this.currentObjective = L("Somehow I've been maneuvered into finding Clint some new shoes.  Alex wears the same shoe size and tosses out shoes regularly...  Hm...  Ew...  Hm...");
                    break;
                ...
                case LoaderQuestState.LinusSniffing5:
                    this.currentObjective = L("I've got Linus recruited to look out for shoes in Alex's trash can, but he warns me that there's something else that might be looking as well...  Who could that be?");
                    break;
                case LoaderQuestState.DisguiseTheShoes:
                    this.currentObjective = L("Somehow I need to disguise these shoes so that Alex doesn't recognize them...  And maybe do something about that smudge.  Perhaps Sam or Sebastian would loan me some shoe polish.");
                    break;
                case LoaderQuestState.GiveShoesToClint:
                    this.currentObjective = L("Give the shoes and the old loader to Clint");
                    break;
                case LoaderQuestState.InstallTheLoader:
                    this.currentObjective = L("Take the fixed loader attachment to the tractor garage.");
                    break;
                case LoaderQuestState.WaitForClint1:
                case LoaderQuestState.WaitForClint2:
                    this.currentObjective = L("Wait for Clint to finish repairing the loader.");
                    break;
                case LoaderQuestState.PickUpLoader:
                    this.currentObjective = L("Clint should be done repairing the loader by now.");
                    break;
            }
        }
        ...
        public override void CheckIfComplete(NPC n, Item? item)
        {
            ...
            {
                this.Spout(n, L("Shoes, yeah man, nice shoes cost a fortune.  My gig at the library barely pays, so I roll around in these supercheapies from Joja.  I color mine every once in a while so they look fresh."));
            }
            ...
            {
                this.Spout(n, L("Ladies' shoes I know.  Men's shoes I don't.  'Cheap' shoes is definitely not something I spend time thinking about.$3"));
            }
            else if (n.Name == "Emily" && this.State < LoaderQuestState.SnagAlexsOldShoes && this.State > LoaderQuestState.TalkToClint)
            {
                this.Spout(n, L("Well, I mostly get my shoes from secondhand stores, but I don't really know about men's shoes.  Have you asked Sam, or Alex?"));
            }
            else if (n.Name == "Sebastian" && this.State < LoaderQuestState.SnagAlexsOldShoes && this.State > LoaderQuestState.TalkToClint)
            {
                this.Spout(n, L("Shoes..  Yeah man, decent shoes cost a fortune.  I always wear black, though, which makes it easy.  Blacken anything and it looks cool."));
            }
            else if (n.Name == "Alex" && this.State < LoaderQuestState.SnagAlexsOldShoes && this.State > LoaderQuestState.TalkToClint)
            {
                ...
                this.Spout(n, L("I got these new shoes yesterday 'cuz my old pair had a brown smudge.#$b#I just threw them into the garbage. I would've donated them but I don't like the idea of some weirdo wearing my shoes, ya know?#$b#What size do I wear?  14EEE. . . .  Wait, why do you ask?"));
            ...
            else if (n.Name == "Linus" && Game1.player.getFriendshipHeartLevelForNPC("Linus") >= 2 && this.State == LoaderQuestState.SnagAlexsOldShoes)
            {
                ...
                this.Spout(n, L(". . . So...  what I think I'm hearing you say is you want me to scout the Mullner's trash can for shoes...#$b#You promise this is for a good cause?  Hm...  Okay.  I'll let you know if I come across them.#$b#I don't want to disturb you, but I'm not the only one nosing around town late at night.  I might not find them first."));
                this.State = LoaderQuestState.LinusSniffing1;
            }
            else if (n.Name == "Linus" && this.State > LoaderQuestState.LinusSniffing1 && this.State <= LoaderQuestState.LinusSniffing5)
            {
                this.Spout(n, L("No luck last night, but I'm keeping an eye out for those shoes!"));
            }
            else if ((n.Name == "Sam" || n.Name == "Sebastian") && this.TryTakeItemsFromPlayer(ObjectIds.AlexesOldShoe))
            {
                string treat = (n.Name == "Sam" ? L("pizza") : L("sashimi"));
                this.Spout(n, LF($"You want to borrow my shoe polish?  That's kindof an odd request but, you know what?  Sure.  Knock yourself out.#$b#There better be some {treat} in this for me somewhere down the road."));
