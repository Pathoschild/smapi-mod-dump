...
namespace NermNermNerm.Stardew.QuestableTractor
{
        ...
        : TractorPartQuest<WatererQuestState>
    {
        ...
        public WatererQuest(WatererQuestController controller)
            : base(controller)
        {
            this.questTitle = L("Fix the waterer");
            this.questDescription = L("I found the watering attachment for the tractor, but it's in bad shape, I should ask around town.");
        }
        ...
        public override void CheckIfComplete(NPC n, Item? item)
        {
            ...
            {
                this.Spout(n, L("Oh, that isn't...  It is!  It's your grandpa's legendary irrigation attachment!  And you *fished* it up you say?  Hah!  Well you would'a had to, wouldn'tya!$1#$b#. . . #$b#Sorry, I gotta compose myself.  You'll want to take this up to the mountain.  Show it to Robin, she can give you the first-hand account.$1#$b#It might not be safe to show it to Demetrius.  I think he's still working through the aftereffects...$s"));
            ...
            else if (n.Name == "Maru")
            {
                switch (this.State)
                {
                    ...
                    case WatererQuestState.GetGoldBars:
                        ...
                        {
                            this.Spout(n, L("Alrighty, I'll get to work on it and have it back to you in a couple days.  I'll just drop it in the mail for you."));
                            this.State = WatererQuestState.WaitForMaruDay1;
                        }
                        else if (itemIsWaterer)
                        {
                            this.Spout(n, L("Have you found some gold bars yet?  Gotta go pretty deep in the mines to get it, but I'm sure you're up for it."));
                        }
                        break;
                    case WatererQuestState.WaitForMaruDay1:
                        this.Spout(n, L("I'm still working on the irrigation system; you should have it day after tomorrow."));
                        break;
                    case WatererQuestState.WaitForMaruDay2:
                        this.Spout(n, L("I'm working through the bugs on the irrigation system; you should have it tomorrow if nothing goes haywire.$3"));
                        break;
                }
            }
        }
        ...
        protected override void SetObjective()
        {
            switch (this.State)
            {
                case WatererQuestState.NoCluesYet:
                    ...
                    break;
                case WatererQuestState.RobinFingered:
                    this.currentObjective = L("Ask Robin about this thing.");
                    break;
                case WatererQuestState.MaruFingered:
                case WatererQuestState.MaruFingeredByDemetrius:
                    this.currentObjective = L("Take it to Maru to see if she'll fix it.");
                    break;
                case WatererQuestState.GetGoldBars:
                    this.currentObjective = LF($"Bring the watering can and {goldBarCount} gold bars to Maru.");
                    break;
                case WatererQuestState.WaitForMaruDay1:
                case WatererQuestState.WaitForMaruDay2:
                    this.currentObjective = L("Just gotta wait - should be in the mail any day now.");
                    break;
                case WatererQuestState.InstallPart:
                    this.currentObjective = L("Bring the fixed waterer to the garage.");
                    break;
            }
        }
        ...
        {
            this.Spout(L("Maru came through!  Time to take it to the garage and water some crops!"));
