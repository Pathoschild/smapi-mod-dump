...
namespace TheLion.Stardew.Professions.Framework.Patches
{
	internal class ProjectileUpdatePatch : BasePatch
	{
		...
		{
			Original = typeof(Projectile).MethodNamed(nameof(Projectile.update));
			...
			NetCharacterRef ___theOneWhoFiredMe, NetFloat ___xVelocity, NetFloat ___yVelocity, GameLocation location)
		{
			try
			{
				...
				if (!firer.IsLocalPlayer || ModEntry.SuperModeIndex != Util.Professions.IndexOf("Desperado")) return;
