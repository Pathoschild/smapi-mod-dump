...
namespace TheLion.Stardew.Common.Extensions
{
	...
	{
		/// <summary>Determine if the calling list contains any instance of the given types.</summary>
		/// <param name="types">The types to search for.</param>
		public static bool ContainsType<T>(this IList<T> list, params Type[] types)
		{
			return list.Any(item => item != null && item.AnyOfType(types));
		}
