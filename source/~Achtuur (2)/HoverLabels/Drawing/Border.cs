...
public class Border
{
    ...
    /// </summary>
    private float Width => Math.Max(FixedWidth, Labels.Max(text => text.Width));
    /// <summary>
    /// Height of the border without taking into account the outside borders. This is the height of the 'inside' part.
    /// </summary>
    private float Height => Labels.Sum(text => text.Height) + BonusHeight;

    private float BonusHeight => (BottomBorder == BorderType.TSection) ? LabelText.Margin() : 0;
    /// <summary>
    /// Width in tiles
    /// </summary>
    ...
    public Border(IEnumerable<LabelText> labels)
    {
        TopBorder = BorderType.TSection;
        BottomBorder = BorderType.TSection;
        Labels = new();
        LabelPositions = new();
        foreach (LabelText label in labels)
            AddLabelText(label);
    }

    public void AddLabelText(IEnumerable<LabelText> label_text)
    {
        foreach (LabelText label in label_text)
            AddLabelText(label);
    }
    ...
    public void AddLabelText(LabelText label_text)
    {
        ...
        {
            height = TileSize / 2 + LabelText.Margin();
        } 
        else
        {
            height = LabelPositions.Last().Y + Labels.Last().Height;
        }
        // x offset is left side of the border, plus the margin
        float x_offset = TileSize/2 + LabelText.Margin();
        LabelPositions.Add(new Vector2(x_offset, height));
        Labels.Add(label_text);
    }
    
    public void AddLabelText(string text)
    {
        AddLabelText(new LabelText(text));
    }
    ...
    private void DrawLabels(SpriteBatch spriteBatch)
    {
        ...
        if (TopBorder == BorderType.TSection)
            extra_offset = Vector2.UnitY * LabelText.Margin()/2;
