...
namespace BNWCore.Patches
{
    internal class CropPatches
    {
            ...
            public class Crop_harvest_Patch
            {
                public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
                {
                    ...
                    for (int i = 0; i < codes.Count; i++)
                    {
                        ...
                        {
                            codes.Insert(i + 2, new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(CropPatches), nameof(CropPatches.BNWCoreHarvest))));
        ...
        public static void BNWCoreHarvest(Crop crop, int xTile, int yTile)
        {
            if (!(Game1.MasterPlayer.mailReceived.Contains("earth_farming_blessing")) || crop.dead.Value || crop.netSeedIndex.Value == crop.indexOfHarvest.Value || (!InternalConfig.RegrowableSeeds && crop.regrowAfterHarvest.Value > -1) || Game1.random.NextDouble() > InternalConfig.SeedChance / 100f)
                return;
            int index;
            ...
            if (index == -1)
                return;
            int amount = Game1.random.Next(InternalConfig.MinSeeds, InternalConfig.MaxSeeds + 1);
            if (amount <= 0)
                return;
