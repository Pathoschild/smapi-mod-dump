    ...
    public class Bomb_Patches
    {
        ...
        public static bool Explode_Prefix(GameLocation __instance, Vector2 tileLocation, ref int radius, Farmer who, ref bool damageFarmers, ref int damage_amount)
        {
            if (Game1.MasterPlayer.mailReceived.Contains("fire_mining_blessing"))
            {
                try
                {
                    damageFarmers = Bomb_Config.DamageFarmers;
                    radius = Convert.ToInt32(radius * Bomb_Config.Radius);
                    if (damage_amount > 0)
                    {
                        damage_amount = Convert.ToInt32(damage_amount * Bomb_Config.Damage);
                    }
                    else
                    {
                        damage_amount = Convert.ToInt32(radius * Game1.random.Next(radius * 6, (radius * 8) + 1) * Bomb_Config.Damage);
                    }
        ...
        public static void ChangedExplode(GameLocation instance, Vector2 tileLocation, int radius, Farmer who)
        {
            if (Game1.MasterPlayer.mailReceived.Contains("fire_mining_blessing"))
            {
                ...
                if (Bomb_Config.CollectMinerals)
                {
