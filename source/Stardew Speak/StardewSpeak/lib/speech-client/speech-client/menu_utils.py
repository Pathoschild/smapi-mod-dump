...
def find_component_containing_mouse(list_of_cmp_rows: list[list[ClickableComponent]]):
...
def list_of_rows(cmps: list[ClickableComponent], y_threshold=25):
    """
...
async def get_active_menu(menu_type: str | None = None):
    menu: BaseMenu | None = await server.request("GET_ACTIVE_MENU")
...
async def click_component(cmp: ClickableComponent):
...
def scroll_commands(page_size=4):
    ...
    return {
        ...
        "page up [<positive_num>]": df_utils.AsyncFunction(
            scroll_up, format_args=lambda **kw: [kw["positive_num"] * page_size]
        ),
        "page down [<positive_num>]": df_utils.AsyncFunction(
            scroll_down, format_args=lambda **kw: [kw["positive_num"] * page_size]
        ),
    }
...
def validate_menu_type(menu_type: str | None, menu: BaseMenu | None):
    if menu_type is not None:
        if menu is None:
            raise InvalidMenuOption(f"Expecting {menu_type}, got None")
        if menu["menuType"] != menu_type:
