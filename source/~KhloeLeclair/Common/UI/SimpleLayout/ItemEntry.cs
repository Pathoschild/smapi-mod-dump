...
namespace Leclair.Stardew.Common.UI.SimpleLayout {
	public struct FallbackItemNode : ISimpleNode {
		...
		public FallbackItemNode(Item value, IModHelper helper, Alignment alignment = Alignment.None, float scale = 4, bool drawQuantity = false, bool drawLabel = false) {
			Value = value;
			Helper = helper;
			Alignment = alignment;
			Scale = scale;
		...
		public Vector2 GetSize(SpriteFont defaultFont, Vector2 containerSize) {
			...
			if (DrawLabel) {
				...
				height = MathHelper.Max(height, size.Y);
			}
