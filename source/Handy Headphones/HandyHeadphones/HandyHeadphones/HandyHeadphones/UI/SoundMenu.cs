...
{
    public class SoundMenu : IClickableMenu
    {
        // Constants
        private const int soundsPerPage = 6;
        ...
        private int maxSongWidth = (int)Game1.dialogueFont.MeasureString("dialogueCharacterClose").X;

        public List<ClickableComponent> soundButtons;
        ...
        public SoundMenu(List<string> sounds, actionOnChoosingListOption chooseAction) : base(0, 0, 0, 0, showUpperRightCloseButton: true)
        {
            ...
            this.soundButtons = new List<ClickableComponent>();

            for (int i = 0; i < soundsPerPage; i++)
            {
                this.soundButtons.Add(new ClickableComponent(new Rectangle(base.xPositionOnScreen + 16, base.yPositionOnScreen + 16 + i * ((base.height - 32) / soundsPerPage), base.width - 32, (base.height - 32) / soundsPerPage + 4), string.Concat(i))
                {
                    myID = i,
                    downNeighborID = -7777,
        ...
        public void PaginateSounds()
        {
            int count = sounds.Count - 1;
            this.pages = new List<List<string>>();
            foreach (string song in sounds)
            {
                int which2 = sounds.Count - 1 - count;
                while (this.pages.Count <= which2 / soundsPerPage)
                {
                    this.pages.Add(new List<string>());
                }
                this.pages[which2 / soundsPerPage].Add(song);
        ...
        protected override void customSnapBehavior(int direction, int oldRegion, int oldID)
        {
            if (oldID >= 0 && oldID != region_forwardButton && oldID != region_backButton)
            {
                ...
                {
                    case 2:
                        if (oldID < soundsPerPage - 1 && this.pages[this.currentPage].Count - 1 > oldID)
                        {
        ...
        public override void receiveLeftClick(int x, int y, bool playSound = false)
        {
            ...
            for (int i = 0; i < this.soundButtons.Count; i++)
            {
                ...
                if (this.soundButtons[i].containsPoint(x, y))
                {
                    //base.exitThisMenu();
        ...
        public override void draw(SpriteBatch b)
        {
            ...
            for (int j = 0; j < this.soundButtons.Count; j++)
            {
                if (this.pages.Count() > 0 && this.pages[this.currentPage].Count() > j)
                {
                    ...
                    IClickableMenu.drawTextureBox(b, Game1.mouseCursors, new Rectangle(384, 396, 15, 15), this.soundButtons[j].bounds.X, this.soundButtons[j].bounds.Y, this.soundButtons[j].bounds.Width, this.soundButtons[j].bounds.Height, this.soundButtons[j].containsPoint(Game1.getOldMouseX(), Game1.getOldMouseY()) ? Color.Wheat : Color.White, 4f, drawShadow: false);
                    if (songName == "Off")
                    {
                        Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(this.soundButtons[j].bounds.X + 32, this.soundButtons[j].bounds.Y + 28), new Rectangle(137, 384, 9, 9), Color.White, 0f, Vector2.Zero, 4f, flipped: false, 0.99f, shadowIntensity: 0f);
                    }
                    else
                    {
                        Utility.drawWithShadow(b, Game1.mouseCursors, new Vector2(this.soundButtons[j].bounds.X + 32, this.soundButtons[j].bounds.Y + 28), new Rectangle(516, 1916, 7, 10), Color.White, 0f, Vector2.Zero, 4f, flipped: false, 0.99f, shadowIntensity: 0f);
                    }
                    ...
                    Utility.drawTextWithShadow(b, songName, Game1.dialogueFont, new Vector2((float)(base.xPositionOnScreen + base.width / 2) - Game1.dialogueFont.MeasureString(songName).X / 2f, this.soundButtons[j].bounds.Y + 20), Game1.textColor);
