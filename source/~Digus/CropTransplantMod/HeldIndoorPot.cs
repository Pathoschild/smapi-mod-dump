...
namespace CropTransplantMod
{
    public class HeldIndoorPot : IndoorPot
    {
        ...
        {
            base.initNetFields();
            base.NetFields.AddField(this._tree);
        }
        ...
        {
            if ((this.hoeDirt.Value.crop != null && !DataLoader.ModConfig.EnablePlacementOfCropsOutsideOutOfTheFarm && !location.IsFarm && !location.CanPlantSeedsHere(this.hoeDirt.Value.crop.netSeedIndex.Value, x / 64, y / 64, false, out var deniedMessage) && location.IsOutdoors))
            {
                Game1.showRedMessage(deniedMessage??Game1.content.LoadString("Strings\\StringsFromCSFiles:HoeDirt.cs.13919"));
                return false;
            }
        ...
        public override void drawWhenHeld(SpriteBatch spriteBatch, Vector2 objectPosition, Farmer f)
        {
            ...
            var x = local.X / 64;
            var y = (local.Y + 64) / 64;
            spriteBatch.Draw(Game1.bigCraftableSpriteSheet, objectPosition, new Rectangle?(Object.getSourceRectForBigCraftable(f.ActiveObject.ParentSheetIndex)), Color.White, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f));
            if (this.hoeDirt.Value.HasFertilizer())
            {
                ...
                spriteBatch.Draw(Game1.mouseCursors, new Vector2((float)((double)objectPosition.X + 4.0), (float)(objectPosition.Y + 52)), this.hoeDirt.Value.GetFertilizerSourceRect(), Color.White, 0.0f, Vector2.Zero, 4f, SpriteEffects.None, Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f)+0.0001f);
            }
        ...
        private void DrawWithOffset(Crop crop, SpriteBatch b, Vector2 tileLocation, Color toTint, float rotation, Vector2 offset, Farmer f)
        {
            ...
            {
                b.Draw(Game1.mouseCursors,  offset + tileLocation, crop.sourceRect, Color.White, 0.0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f) + 0.0002f);
            }
            else
            {
                ...
                b.Draw(Game1.cropSpriteSheet,  offset + tileLocation, sourceRect, toTint, rotation, new Vector2(8f, 24f), 4f, crop.flip.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f) + 0.0002f);
                if (crop.tintColor.Value.Equals((object)Color.White) || crop.currentPhase.Value != crop.phaseDays.Count - 1 || crop.dead.Value)
                    return;
                b.Draw(Game1.cropSpriteSheet, offset + tileLocation, new Rectangle?(new Rectangle((crop.fullyGrown.Value ? (crop.dayOfCurrentPhase.Value <= 0 ? 6 : 7) : crop.currentPhase.Value + 1 + 1) * 16 + (crop.rowInSpriteSheet.Value % 2 != 0 ? 128 : 0), crop.rowInSpriteSheet.Value / 2 * 16 * 2, 16, 32)), crop.tintColor.Value, rotation, new Vector2(8f, 24f), 4f, crop.flip.Value ? SpriteEffects.FlipHorizontally : SpriteEffects.None, Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f) + 0.0003f);
            }
        }
        ...
        public void drawTree(Tree treeToDraw, SpriteBatch spriteBatch, Vector2 tileLocation, Farmer f)
        {
            ...
            if (treeToDraw.growthStage.Value < 5)
            {
                ...
                spriteBatch1.Draw(texture, tileLocation + new Vector2(32,96), sourceRectangle, white, (float)shakeRotation, origin, (float)num1, (SpriteEffects)num2, Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f) + 0.0002f);
            }
            else
            {
                if (!treeToDraw.stump.Value)
                {
                    ...
                    SpriteBatch spriteBatch1 = spriteBatch;
                    Microsoft.Xna.Framework.Rectangle? sourceRectangle = new Microsoft.Xna.Framework.Rectangle?(STree.treeTopSourceRect);
                    Color color = Color.White;
                    Vector2 origin = new Vector2(24f, 96f);
                    double num1 = 4.0;
                    int num2 = treeToDraw.flipped.Value ? 1 : 0;
                    double num3 = Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f) + 0.0003f;
                ...
                if (treeToDraw.health.Value > -99.0)
                {
                    ...
                    double num4 = Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f) + 0.0002f;
                ...
                if (treeToDraw.stump.Value && treeToDraw.health.Value < 4.0 && treeToDraw.health.Value > -99.0)
                {
                    ...
                    double num2 = 4.0;
                    int num3 = treeToDraw.flipped.Value ? 1 : 0;
                    double num4 = Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f) + 0.0004f;
                    spriteBatch1.Draw(texture, tileLocation + new Vector2(0, 32), sourceRectangle, color, (float)num1, zero, (float)num2, (SpriteEffects)num3, (float)num4);
                }
            }
            foreach (Leaf leaf in leaves)
            {
                ...
                double num1 = 4.0;
                int num2 = 0;
                double num3 = Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f) + 0.0005f;
        ...
        public void drawFruitTree(FruitTree treeToDraw, SpriteBatch spriteBatch, Vector2 tileLocation, Farmer f)
        {
            ...
            var spriteRowNumber = treeToDraw.GetSpriteRowNumber();
            Texture2D texture = treeToDraw.texture;
            if (treeToDraw.growthStage.Value < 4)
            {
                ...
                {
                    case 0:
                        rectangle = new Rectangle(0, spriteRowNumber * 5 * 16, 48, 80);
                        break;
                    case 1:
                        rectangle = new Rectangle(48, spriteRowNumber * 5 * 16, 48, 80);
                        break;
                    case 2:
                        rectangle = new Rectangle(96, spriteRowNumber * 5 * 16, 48, 80);
                        break;
                    default:
                        rectangle = new Rectangle(144, spriteRowNumber * 5 * 16, 48, 80);
                        break;
                }
                SpriteBatch spriteBatch1 = spriteBatch;
                ...
                double num1 = 4.0;
                int num2 = treeToDraw.flipped.Value  ? 1 : 0;
                double num3 = Math.Max(0.0f, (float) (f.StandingPixel.Y + 2) / 10000f) + 0.0002f;
            ...
            else
            {
                if (!treeToDraw.stump.Value)
                {
                    ...
                    SpriteBatch spriteBatch1 = spriteBatch;
                    Rectangle? sourceRectangle = new Rectangle?(new Rectangle((12 + (treeToDraw.IgnoresSeasonsHere() ? 1 : Utility.getSeasonNumber(Game1.currentSeason)) * 3) * 16, spriteRowNumber * 5 * 16, 48, 64));
                ...
                {
                    SpriteBatch spriteBatch1 = spriteBatch;
                    Rectangle? sourceRectangle = new Rectangle?(new Rectangle(384, spriteRowNumber * 5 * 16 + 48, 48, 32));
                    Color color = treeToDraw.struckByLightningCountdown.Value > 0 ? Color.Gray : Color.White;
                    double num1 = 0.0;
                    ...
                    int num3 = treeToDraw.flipped.Value  ? 1 : 0;
                    double num4;
                    num4 = Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f) + 0.0002f;
            ...
            foreach (Leaf leaf in leaves)
            {
                SpriteBatch spriteBatch1 = spriteBatch;
                Vector2 local = Game1.GlobalToLocal(Game1.viewport, leaf.position);
                Rectangle? sourceRectangle = new Rectangle?(new Rectangle((24 + Utility.getSeasonNumber(Game1.currentSeason)) * 16, spriteRowNumber * 5 * 16, 8, 8));
                Color white = Color.White;
                double rotation = (double)leaf.rotation;
                Vector2 zero = Vector2.Zero;
                double num1 = 4.0;
                int num2 = 0;
                double num3 = Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f) + 0.0004f;
                spriteBatch1.Draw(texture, local, sourceRectangle, white, (float)rotation, zero, (float)num1, (SpriteEffects)num2, (float)num3);
            }
        }
        ...
        public void drawTeaBush(Bush bushToDraw, SpriteBatch spriteBatch, Vector2 tileLocation, Farmer f)
        {
            ...
            float layerDepth = Math.Max(0.0f, (float)(f.StandingPixel.Y + 2) / 10000f) + 0.0002f;
