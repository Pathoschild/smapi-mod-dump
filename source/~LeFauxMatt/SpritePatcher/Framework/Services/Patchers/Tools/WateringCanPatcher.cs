...
/// <summary>Patches for <see cref="WateringCan" /> draw methods.</summary>
internal sealed class WateringCanPatcher : BasePatcher
{
    /// <summary>Initializes a new instance of the <see cref="WateringCanPatcher" /> class.</summary>
    ...
    public WateringCanPatcher(
        ...
        this.Patches.Add(
            this.Id,
            new SavedPatch(
                AccessTools.DeclaredMethod(typeof(WateringCan), nameof(WateringCan.drawInMenu)),
                AccessTools.DeclaredMethod(typeof(BasePatcher), nameof(BasePatcher.DrawInMenu)),
                PatchType.Transpiler));

    /// <inheritdoc />
    public override AllPatches Type => AllPatches.PatchedWateringCan;
}