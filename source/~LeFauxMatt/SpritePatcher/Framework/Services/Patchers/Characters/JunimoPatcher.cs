...
/// <summary>Patches for <see cref="Junimo" /> draw methods.</summary>
internal sealed class JunimoPatcher : BasePatcher
{
    /// <summary>Initializes a new instance of the <see cref="JunimoPatcher" /> class.</summary>
    ...
    public JunimoPatcher(
        ...
        this.Patches.Add(
            this.Id,
            new SavedPatch(
                AccessTools.DeclaredMethod(typeof(Junimo), nameof(Junimo.draw)),
                AccessTools.DeclaredMethod(typeof(BasePatcher), nameof(BasePatcher.Draw)),
                PatchType.Transpiler),
            new SavedPatch(
                AccessTools.DeclaredMethod(typeof(Junimo), nameof(Junimo.DrawShadow)),
                AccessTools.DeclaredMethod(typeof(BasePatcher), nameof(BasePatcher.DrawShadow)),
                PatchType.Transpiler));

    /// <inheritdoc />
    public override AllPatches Type => AllPatches.PatchedJunimo;
}