...
/// <summary>Patches for <see cref="ResourceClump" /> draw methods.</summary>
internal sealed class ResourceClumpPatcher : BasePatcher
{
    /// <summary>Initializes a new instance of the <see cref="ResourceClumpPatcher" /> class.</summary>
    ...
    public ResourceClumpPatcher(
        ...
        this.Patches.Add(
            this.Id,
            new SavedPatch(
                AccessTools.DeclaredMethod(typeof(ResourceClump), nameof(ResourceClump.draw)),
                AccessTools.DeclaredMethod(typeof(BasePatcher), nameof(BasePatcher.Draw)),
                PatchType.Transpiler));

    /// <inheritdoc />
    public override AllPatches Type => AllPatches.PatchedResourceClump;
}