...
public interface IContentPatcherApi
{
    ...
    *********/
    ...
    ///     <see cref="IGameLoopEvents.GameLaunched" /> event.
    /// </remarks>
    ...
    *********/
    ...
    ///     The format version for which to parse conditions, used to ensure forward compatibility with
    ///     future Content Patcher versions. See <c>Format</c> in the Content Patcher token documentation.
    /// </param>
    /// <param name="assumeModIds">
    ///     <para>
    ///         The unique IDs of mods whose custom tokens to allow in the <paramref name="rawConditions" />. You don't need
    ///         to list the mod identified by <paramref name="manifest" />, mods listed as a required dependency in the
    ///         <paramref name="manifest" />, or mods identified by a <c>HasMod</c> condition in the
    ///         <paramref name="rawConditions" />.
    ///     </para>
    ///     <para>
    ///         NOTE: this is meant to prevent mods from breaking if a player doesn't have a required mod installed. You
    ///         shouldn't simply list all installed mods, and parsing conditions will still fail if a mod isn't installed
    ///         regardless of the listed mod IDs.
    ///     </para>
    /// </param>
    IManagedConditions ParseConditions(
        IManifest manifest,
        IDictionary<string, string?>? rawConditions,
    ...
    /// </param>
    /// <param name="getValue">
    ///     A function which returns the current token value. If this returns a null or empty list, the
    ///     token is considered unavailable in the current context and any patches or dynamic tokens using it are disabled.
    /// </param>
