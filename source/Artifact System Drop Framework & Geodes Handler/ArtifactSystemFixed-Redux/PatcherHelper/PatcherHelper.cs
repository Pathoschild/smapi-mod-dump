 ...
 *    ####################   ATTENTION   ####################
 *    
 *    PLEASE SET <LangVersion>8.0</LangVersion> IN YOUR .csproj FILE !!!!
 * 
 *    (Higher version is okay, but AT MINIMUM set it to 8.0 !!)
 * 
 */
...
namespace PatcherHelper
    ...
    public static class Meta
        {
        public static string Revision = "R73";
        }
    ...
    public class PatchTypeAttribute : Attribute
        ...
        public static PatchType FromPatch(HarmonyMethod patch) {
            if (patch.method.GetCustomAttribute<PatchTypeAttribute>() is PatchTypeAttribute pt_attr)
                return pt_attr.PType;
    ...
    public static class CodeInstructionExtensions
        ...
        public static string Repr(this CodeInstruction instruction) {
            ...
            string oprt_s = oprt?.ToString() ?? "null";
            if (oprt_s.Contains("ConstructorInfo")) {
                return $"{opc} | ({oprt_s}) {opr} <{(opr as ConstructorInfo).DeclaringType.Name}>";
                }
            else {
        ...
        public int Execute(out Dictionary<string, Exception> failures) {
            ...
            foreach (var i in PatchQueue.Where(i => i.enabled)) {
                try {
                    ...
                    if (FailFast) throw;
                    culprit = Util.JoinNonNulls(" OR ", i.prefix?.methodName, i.postfix?.methodName, i.transpiler?.methodName);
                    failures[culprit] = ex;
                    }
                }
    ...
    public class HarmonyPatcherHelper
        {
        public static bool AUDIT = false;
        ...
        public bool TryPatching(
            ...
            if (AUDIT) harmony.AuditPatches(original);
            string poi = $"{original.DeclaringType.Namespace}.{original.DeclaringType.Name}.{original.Name}";
            var patch_typess = Util.SelectIfNotNull(prefix, "prefix", postfix, "postfix", transpiler, "transpiler");
            string patch_type = string.Join(",", patch_typess.Select(o => (string)o));
            try {
        ...
        public bool TryPatching(
            ...
            HarmonyMethod mprefix = (prefix is null) ? null : new HarmonyMethod(PatcherClass, prefix);
            HarmonyMethod mpostfix = (postfix is null) ? null : new HarmonyMethod(PatcherClass, postfix);
            HarmonyMethod mtransp = (transpiler is null) ? null : new HarmonyMethod(PatcherClass, transpiler);
    ...
    /// </summary>
    public class InstructionsWalker : IEnumerable<CodeInstruction>
        {
        ...
        /// <summary>
        /// <para>Instantiates an InstructionsWalker with internal buffer populated by the given sequence.</para>
        /// <para>Please note that this class implements the <see cref="IEnumerable{T}"/> interface, so you can
        /// return an instance of this class directly from a transpiler static method.</para>
        /// </summary>
        /// <param name="instructions">A sequence of CIL to 'walk' through</param>
        public InstructionsWalker(IEnumerable<CodeInstruction> instructions) {
            Instructions = instructions.ToList();
            }


        #region Methods for implementing IEnumerable<CodeInstruction>
        ...
        public CodeInstruction CurrentOp(WantOp assert_op = null) {
            ...
            if (assert_op?.NotMatches(op) ?? false)
        ...
        /// <returns><c>this</c> for Fluent chaining</returns>
        public InstructionsWalker FindNextMatch(out int where, IWantOp to_match, bool relative_pos = true, int? assert_where = null) {
            int? findloc = null;
            for (int i = CurrentLocation; i < Instructions.Count; i++) {
        ...
        public InstructionsWalker GoForward(int N, WantOp assert_op = null) {
            ...
            return GoTo(newpos, assert_op: assert_op);
            }
        ...
        public InstructionsWalker GoFetchOp(out CodeInstruction fetchedOp, WantOp assert_op = null, bool move_pos = true) {
            ...
            return move_pos ? GoForward(1) : this;
            }
        ...
        public InstructionsWalker GoFind(IWantOp[] wantOps, bool pos_after = false, bool exception_on_notfound = true) {
            int maxLoc = Instructions.Count - wantOps.Length;
            int? foundAt = null;
            ...
            GoTo(foundAt.Value);
            return pos_after ? GoForward(wantOps.Length) : this;
            }
        ...
        /// <para>If <c>wantOps</c> is found, it will return <c>this</c> to allow Fluent chaining of commands.</para>
        /// <para>Finding position can be retrieved using <c>CurrentLocation</c></para></returns>
        public InstructionsWalker GoFind(IWantOp wantop, bool pos_after = false, bool exception_on_notfound = true) =>
            GoFind(new IWantOp[] { wantop }, pos_after, exception_on_notfound);
        ...
        public InstructionsWalker NOPify(
            ...
            int begin_pos = (absolute_begin_pos ?? CurrentLocation) + relative_begin_pos;

            int end_pos = Util.CountNull(absolute_end_pos, relative_end_pos, length) switch {
                // 2 null's mean exactly one of the three args was given
                2 when length.HasValue => begin_pos + length.Value,
                2 when relative_end_pos.HasValue => CurrentLocation + relative_end_pos.Value,
                2 when absolute_end_pos.HasValue => absolute_end_pos.Value,
                _ => throw new ArgumentException("ONE AND ONLY ONE OF absolute_end_pos|relative_end_pos|length must be given!")
                };
            ...
            return move_pos ? GoTo(end_pos) : this;
            }

        /// <summary>
        /// Replace instructions from CurrentLocation for a certain number of instructions (which itself will not be replaced)
        /// </summary>
        /// <param name="length">How many instructions to NOPify</param>
        /// <param name="move_pos">If true (explicit), then move CurrentPos to the instruction following the sequence of NOPs</param>
        /// <param name="copy">Combination of OpCopy flags to specify what metaop to copy</param>
        ...
        /// <exception cref="IndexOutOfRangeException">Thrown if 'length' goes beyond Intructions array</exception>
        public InstructionsWalker NOPify(int length, bool move_pos = true, OpCopy copy = 0, int? assert_count = null, WantOp assert_next = null) {
            int stop_pos = CurrentLocation + length;
            if (stop_pos >= Instructions.Count) throw new IndexOutOfRangeException("length is beyond Instructions maximum");
            bool copy_labels = copy.HasFlag(OpCopy.Labels);
            bool copy_blocks = copy.HasFlag(OpCopy.Blocks);
            ...
            for (int loc = CurrentLocation; loc < stop_pos; loc++) {
                ...
                if (copy_blocks) new_op.blocks = orig_op.blocks;
        ...
        /// <param name="assert_next">Assert the next op following the NOPified block</param>
        /// <returns><c>this</c> for Fluent chaining</returns>
        /// <exception cref="PatchingFailureException">Thrown if the 'until' WantOp cannot be found</exception>
        public InstructionsWalker NOPify(IWantOp until, bool move_pos = true, OpCopy copy = 0, int? assert_count = null, WantOp assert_next = null) {
            ...
            return NOPify(stop_pos - CurrentLocation, move_pos, copy, assert_count, assert_next);
            }
        ...
        public InstructionsWalker ReplaceAt(
            ...
            return move_pos ? GoTo(pos + 1) : this;
            }
        ...
        public InstructionsWalker GoDump(out List<string> result, int N, bool move_pos = true) {
            ...
            return move_pos ? GoForward(N) : this;
            }

        public InstructionsWalker GoDumpAddTo(ref List<string> result, int N, bool move_pos = true) {
            result.AddRange(
                Instructions
                    .Skip(CurrentLocation)
                    .Take(N)
        ...
        public InstructionsWalker InsertAt(IEnumerable<CodeInstruction> instructions, bool move_pos = false) {
            Instructions.InsertRange(CurrentLocation, instructions);
            return move_pos ? GoForward(instructions.Count()) : this;
            }

        public InstructionsWalker InsertAt(CodeInstruction instruction, bool move_pos = false) {
            Instructions.Insert(CurrentLocation, instruction);
            return move_pos ? GoForward(1) : this;
            }
        }
    ...
    internal static class Util
        ...
        /// <summary>
        /// Given a sequence of (possibly null) strings, return a Join-ed string of all the non-null strings
        /// </summary>
        /// <param name="sep">Joining separator</param>
        /// <param name="args"></param>
        /// <returns>Joined string</returns>
        internal static string JoinNonNulls(string sep, params string[] args) => string.Join(sep, args.Where(o => o != null));

        /// <summary>
        /// Given pairs of objects (o0, o1, o2, o3, and so on), will return a list of odd-numbered objects it the preceding even-numbered object is not null.
        /// </summary>
        /// <param name="args">Pairs of objects to select from.</param>
        /// <returns>Selected objects</returns>
        /// <exception cref="ArgumentException">Raised if the number of objects are not even</exception>
        internal static IEnumerable<object> SelectIfNotNull(params object[] args) {
            if ((args.Length % 2) != 0)
                throw new ArgumentException("Need even number of args!");
            var result = new List<object>();
            for (int i = 0; i < args.Length; i += 2) {
                if (args[i] != null) result.Add(args[i + 1]);
                }
            return result;
            }
        }
    ...
    public interface IWantOp
        {
        public bool Matches(CodeInstruction instruction);
        public bool NotMatches(CodeInstruction instruction);
        }
    ...
    public class WantOp : IWantOp
        ...
        // IMPORTANT: If you add a new type in the "Type" column, something that has not existed before, don't forget to add the proper comparer
        ...
        private static readonly List<HashSet<OpCode>> Equivalents = new List<HashSet<OpCode>> {
            ...
            new HashSet<OpCode>() {OpCodes.Stloc, OpCodes.Stloc_S}
            };
        ...
        public bool Matches(CodeInstruction inst) {
            ...
            if (OperandType == typeof(double))
                return (double)inst.operand == (double)OperandValue;
        ...
        internal static WantOp Ldc_R8(double value) => new WantOp(OpCodes.Ldc_R8, value);
        internal static WantOp Ldstr(string value) => new WantOp(OpCodes.Ldstr, value);
        internal static WantOp Newobj(string @class) => new WantOp(OpCodes.Newobj, @class);
        internal static WantOp Stfld(string fieldname) => new WantOp(OpCodes.Stfld, fieldname);
        internal static WantOp Stsfld(string fieldname) => new WantOp(OpCodes.Stsfld, fieldname);
        #endregion
        }



    public class AnyWantOpOf : IWantOp
        {
        readonly IWantOp[] WantOps;

        /// <summary>
        /// <para>Contain several <c>WantOp</c> objects to enable aggregate AnyMatching or NoneMatching</para>
        /// <para>Can be nested (i.e., contains a series of <c>AnyWantOpOf</c> instances)... but why??</para>
        /// </summary>
        /// <param name="wantops"></param>
        public AnyWantOpOf(params IWantOp[] wantops) {
            WantOps = wantops;
            }

        /// <summary>
        /// Try find a match to <c>instruction</c> among several <c>WantOp</c>s
        /// </summary>
        /// <param name="instruction">The instruction to find a match against</param>
        /// <returns><c>true</c> if <em>any</em> of the contained <c>WantOp</c>s match</returns>
        public bool Matches(CodeInstruction instruction) => WantOps.Any(w => w.Matches(instruction));

        /// <summary>
        /// Try to ascertain that none of the contained <c>WantOp</c>s match against <c>instruction</c>
        /// </summary>
        /// <param name="instruction">The instruction to find a match against</param>
        /// <returns><c>true</c> if <em>None</em> of the contained <c>WantOp</c>s match</returns>
        public bool NotMatches(CodeInstruction instruction) => WantOps.All(w => w.NotMatches(instruction));
        }
    ...
    internal class WantOpCache
        ...
        internal WantOp
            ...
            conv_u8 = new WantOp(OpCodes.Conv_U8),
    ...
    /// <summary>
    /// A WantOpCache that contains, in addition to the cached ops of WantOp, commonly found ops for the game Stardew Valley
    /// </summary>
    internal class WantOpCache_SDV : WantOpCache
        {
        internal WantOp
            ...
            new_random = WantOp.Newobj("System.Random")
            ;
        }
