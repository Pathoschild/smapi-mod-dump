...
{
    public class Drill : PickaxeExtended, IEnergyManagerProvider
    {
        ...
        public Drill(BasicItemInformation ItemInfo, int UpgradeLevel, Texture2DExtended WorkingTexture)
        {
            this.info = ItemInfo;
            ...
            if (@object != null)
            {
                if (@object.Name.Equals("Stone"))
                {
                    ...
                    if (!location.Name.StartsWith("UndergroundMine"))
                    {
                        ...
                        ModCore.ModHelper.Reflection.GetMethod(location,"breakStone").Invoke(@object.ParentSheetIndex, num1, num2, who, new Random((int)Game1.stats.DaysPlayed + (int)Game1.uniqueIDForThisGame / 2 + num1 * 4000 + num2));
                    }
                    else
        ...
        {
            return new Drill(this.info.Copy(), this.UpgradeLevel, this.workingTexture.Copy());
        }
