...
namespace Revitalize.Objects
{
    ...
    public class Table : Decoration
    {
        ...
        public Table(int which, Vector2 tile, bool isRemovable = true)
        {
        ...
        public Table(Vector2 tile, string fileName, bool isRemovable = true, int which=0)
        {
            removable = isRemovable;
            // this.thisType = GetType();
            this.tileLocation = tile;
            ...
            if (TextureSheet == null)
            {
                TextureSheet = Game1.content.Load<Texture2D>(Path.Combine("Revitalize", "Furniture","Tables","Graphics",fileName));
                texturePath = "TileSheets\\furniture";
            }
            Dictionary<int, string> dictionary = Game1.content.Load<Dictionary<int, string>>(Path.Combine("Revitalize", "Furniture","Tables", "Data", fileName));
            string[] array = dictionary[which].Split(new char[]
            {
                '/'
            });
