...
namespace Revitalize.Objects
{
    ...
    /// </summary>
    public class shopObject : CoreObject
    {
        ...
        public shopObject(bool f)
        {
            //does nothng
        }

        public shopObject(int which, Vector2 tile, List<Item> shopInventory, string TexturePath )
        {
        ...
        public override bool clicked(StardewValley.Farmer who)
        {
            ...
            if (this.heldObject == null && (who.ActiveObject == null || !(who.ActiveObject is shopObject)))
            {
        ...
        {
            if ((this.Decoration_type == 11 || this.Decoration_type == 5) && this.heldObject == null && !dropIn.bigCraftable && (!(dropIn is shopObject) || ((dropIn as shopObject).getTilesWide() == 1 && (dropIn as shopObject).getTilesHigh() == 1)))
            {
                ...
                //  Log.AsyncO(getDefaultBoundingBoxForType((dropIn as shopObject).Decoration_type));
                ...
                for (int i = 0; i < this.boundingBox.Width / Game1.tileSize; i++)
                {
                    for (int j = 0; j < this.boundingBox.Height / Game1.tileSize; j++)
                    {
                        ...
                        foreach (KeyValuePair<Vector2, StardewValley.Object> something in l.objects)
                        {
                            ...
                            if ((obj.GetType()).ToString().Contains("shopObject"))
                            {
                                shopObject current = (shopObject)obj;
            ...
            else
            {
                ...
                for (int i = 0; i < this.boundingBox.Width / Game1.tileSize; i++)
                {
                    for (int j = 0; j < this.boundingBox.Height / Game1.tileSize; j++)
                    {
                        ...
                        vector.Y += (float)(Game1.tileSize / 2);
                        /*
                        foreach (shopObject current in (l as FarmHouse).shopObject)
                        {
                            if (current.Decoration_type == 11 && current.getBoundingBox(current.tileLocation).Contains((int)vector.X, (int)vector.Y) && current.heldObject == null && this.getTilesWide() == 1)
                            {
                                bool result = true;
        ...
        public override bool placementAction(GameLocation location, int x, int y, StardewValley.Farmer who = null)
        {
            ...
            if (location is FarmHouse)
            {
                ...
                foreach (KeyValuePair<Vector2, StardewValley.Object> c in location.objects)
                {
                    ...
                    if (((ehh.GetType()).ToString()).Contains("shopObject"))
                    {
                        shopObject current2 = (shopObject)ehh;
        ...
        public override void draw(SpriteBatch spriteBatch, int x, int y, float alpha = 1f)
        {
            ...
            {
                if (this.heldObject is shopObject)
                {
                    (this.heldObject as shopObject).drawAtNonTileSpot(spriteBatch, Game1.GlobalToLocal(Game1.viewport, new Vector2((float)(this.boundingBox.Center.X - Game1.tileSize / 2), (float)(this.boundingBox.Center.Y - (this.heldObject as shopObject).sourceRect.Height * Game1.pixelZoom - Game1.tileSize / 4))), (float)(this.boundingBox.Bottom - 7) / 10000f, alpha);
                    return;
                }
        ...
        {
            shopObject shopObject = new shopObject(this.parentSheetIndex, this.tileLocation, this.inventory, this.texturePath);
