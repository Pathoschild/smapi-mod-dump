...
namespace SmartBuilding.Utilities
{
    public class PlayerUtils
    {
        ...
        /// <param name="shouldLog">Whether or not to log. This is forced true by <see cref="StardewModdingAPI.LogLevel.Alert"/>, <see cref="StardewModdingAPI.LogLevel.Error"/>, and <see cref="StardewModdingAPI.LogLevel.Warn"/>.</param>
        public void RefundItem(Item item, string reason = "Something went wrong", LogLevel logLevel = LogLevel.Trace, bool shouldLog = false)
        {
            Game1.player.addItemByMenuIfNecessary(item.getOne());

            if (shouldLog || logLevel == LogLevel.Debug || logLevel >= LogLevel.Error)
                logger.Log($"{reason} {I18n.SmartBuilding_Error_Refunding_RefundingItemToPlayerInventory()} {item.Name}", logLevel);
        }
    }
}