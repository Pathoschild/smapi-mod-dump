...
namespace PlayerCoordinates.Utilities
{
    ...
    {
        private static readonly int _xPosXOffset = 14;
        private static readonly int _xPosYOffset = 8;
        private static int _yPosXOffset = _xPosXOffset;
        private static int _yPosYOffset = _xPosYOffset + 36;

        public static void Draw(Vector2 hudPos, Coordinates currentCoords, Texture2D coordinateBox,
            RenderedHudEventArgs world)
        {
#if ANDROID
            hudPos = new Vector2(125, 9); // This is bad but we want to hardcode the HUD position on Android.
            var topTextPosition = new Vector2(148, 17);
            var bottomTextPosition = new Vector2(topTextPosition.X, topTextPosition.Y + 36);
#else
