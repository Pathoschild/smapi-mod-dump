...
namespace LoveOfCooking.GameObjects
{
	public interface ICookingSkillAPI
	{
		...
		int CalculateExperienceGainedFromCookingItem(Item item, int ingredientsCount, bool apply);
	}
	...
	public class CookingSkillAPI : ICookingSkillAPI
	{
		...
		public int GetExperienceRequiredForLevel(int level)
		{
			...
			return level > 0
				? skill.ExperienceCurve[level] - skill.ExperienceCurve[level - 1]
				: skill.ExperienceCurve[0];
		}
		...
		{
			return level > 0
				? this.GetSkill().ExperienceCurve[level - 1]
				: 0;
		}
		...
		public int CalculateExperienceGainedFromCookingItem(Item item, int ingredientsCount, bool apply)
		{
			...
			var dailyBonus = ModEntry.FoodCookedToday.ContainsKey(item.ParentSheetIndex) ? 0 : 12;
