        ...
        *********/
        ...
        public List<ISalable> GetCarpenterStock(bool unfiltered = false)
        {
            ...
            else
            {
                ...
                {
                    case 0:
                        stock.Add(GetRandomFurniture(r, stock, 1296, 1391));
                        stock.Add(GetRandomFurniture(r, stock, 416, 537));
                        break;
                    case 1:
                        stock.AddRange(new[]
                        {
                    ...
                    case 4:
                        stock.AddRange(new[]
                        {
                            GetRandomFurniture(r, stock, 1296, 1391),
                            GetRandomFurniture(r, stock, 1296, 1391)
                        });
                        break;
                    case 5:
                        stock.AddRange(new[]
                        {
                            GetRandomFurniture(r, stock, 1443, 1450),
                            GetRandomFurniture(r, stock, 288, 313)
                        });
                        break;
                    case 6:
                        stock.AddRange(new[]
                        {
                            GetRandomFurniture(r, stock, 1565, 1607),
                            GetRandomFurniture(r, stock, 12, 129)
                        });
                        break;
                }

                stock.Add(GetRandomFurniture(r, stock));
                stock.Add(GetRandomFurniture(r, stock));
                while (r.NextDouble() < 0.25)
                    stock.Add(GetRandomFurniture(r, stock, 1673, 1815));
        ...
        private List<Item> GetAllFurniture()
        {
            ...
            foreach (var keyValuePair in Content.Load<Dictionary<int, string>>("Data\\Furniture",
        ...
        private Furniture GetRandomFurniture(Random r, List<ISalable> stock, int lowerIndexBound = 0,
            int upperIndexBound = 1462)
        {
            var dictionary = Content.Load<Dictionary<int, string>>("Data\\Furniture", ContentSource.GameContent);
            int num;
            do
            {
        ...
        public Dictionary<ISalable, int[]> GetFishShopStock(Farmer who, bool unfiltered = false)
        {
            ...
            if (unfiltered)
                foreach (var keyValuePair in Game1.objectInformation)
                    if (Game1.objectInformation[keyValuePair.Key].Contains("Fish -4"))
                    {
                        var strArray = Game1.objectInformation[keyValuePair.Key].Split('/');
        ...
        public List<ISalable> GetSaloonStock(bool unfiltered = false)
        {
            ...
            if (Game1.dishOfTheDay.Stack > 0 && !unfiltered)
                ...
                // 194 - 239
                for (var i = 194; i < 240; i++)
                {
                    var parentSheetIndex = i;
                    if (parentSheetIndex == 217)
                        parentSheetIndex = 216;
                    stock.Add(new Object(Vector2.Zero, parentSheetIndex,
                        Game1.random.Next(1, 4 + (Game1.random.NextDouble() < 0.08 ? 10 : 0))));
                }
            ...
            if (unfiltered)
            {
                ...
                foreach (var keyValuePair in Game1.objectInformation)
                    if (Game1.objectInformation[keyValuePair.Key].Contains("/Arch/"))
                    {
                        ...
                        else if (keyValuePair.Key == 96 || keyValuePair.Key == 97 || keyValuePair.Key == 98 ||
                                 keyValuePair.Key == 99)
                            stock.Add(new Object(keyValuePair.Key, int.MaxValue, false, 5000));
                        else
                            stock.Add(new Object(keyValuePair.Key, int.MaxValue, false,
                                Convert.ToInt32(strArray[1]) * 3));
                    }
            }
            ...
            return new List<Object>
            {
                ...
                new Object(100, 1, false, 2000)
                {
                    ...
                    Type = locationName.Equals("Big Coop") || locationName.Equals("Deluxe Coop")
                        ? null
                        : "You gotta be in a Big Coop"
                },
                new Object(100, 1, false, 4000)
                {
                    name = "Sheep", Type = locationName.Equals("Deluxe Barn") ? null : "You gotta be in a Deluxe Barn"
                },
                new Object(100, 1, false, 4000)
                {
                    name = "Rabbit", Type = locationName.Equals("Deluxe Coop") ? null : "You gotta be in a Deluxe Coop"
                },
                new Object(100, 1, false, 8000)
                    {name = "Pig", Type = locationName.Equals("Deluxe Barn") ? null : "You gotta be in a Deluxe Barn"}
            };
        }

        public void FinishAllBundles()
        {
            foreach (var bundle in ((CommunityCenter) Game1.getLocationFromName("CommunityCenter")).bundles.Pairs)
                for (var index = 0; index < bundle.Value.Length; ++index)
                    bundle.Value[index] = true;
        }

        public List<ISalable> GetJunimoStock()
        {
            var junimoItems = new List<ISalable>();
            var junContent = Data.ReadJsonFile<Dictionary<int, string>>("assets\\bundles.json");
            var bundleInfo = ((CommunityCenter) Game1.getLocationFromName("CommunityCenter")).bundles.Pairs.ToArray();
            BundleToAreaDictionary = new Dictionary<int, int>();

            foreach (var kvp in junContent)
            {
                var id = kvp.Key;
                ...
                if (parts.Length >= 7)
                {
                    ...
                    var category = int.Parse(parts[3].Split(' ')[1]);
                    var bundleIndex = int.Parse(parts[4]);
                    var area = parts[5];

                    var o = new Object(id, 1, true, price)
                    {
                        Category = category,
                        Name = name,
                    ...
                    BundleToAreaDictionary.Add(bundleIndex, GetAreaNumberFromName(area));

                    foreach (var bundle in bundleInfo)
                        if (bundle.Key == bundleIndex)
                            foreach (var i in bundle.Value)
                                if (!i)
                                {
                                    junimoItems.Add(o);
                                    break;
                ...
                {
                    Monitor.Log($"Parts not >= 6: {kvp.Key}: {kvp.Value}", LogLevel.Error);
                }
            }
        ...
        {
            var c = (CommunityCenter) Game1.getLocationFromName("CommunityCenter");
            ...
            var area = BundleToAreaDictionary[bundleId];
            if (!c.isJunimoNoteAtArea(area))
                c.addJunimoNote(area);

            // Add the bundle.
            for (var index = 0; index < c.bundles[bundleId].Length; ++index)
