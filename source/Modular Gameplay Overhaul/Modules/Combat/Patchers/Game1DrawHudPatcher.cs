...
internal sealed class Game1DrawHudPatcher : HarmonyPatcher
{
    ...
    private static void DrawHealthBarSubroutine(Vector2 topOfBar)
    {
        ...
        DrawShieldHealth(topOfBar, healthBarRect);
    }
    ...
    internal static void DrawShieldHealth(Vector2 topOfBar, Rectangle healthBarRect)
    {
        ...
        shieldBarRect.Height = (int)(CombatModule.State.YobaShieldHealth / (player.maxHealth * 0.5) * fullBarHeight * 0.5);
        ...
        {
            Game1.drawWithBorder(
                Math.Max(0, CombatModule.State.YobaShieldHealth) + "/" + (int)(player.maxHealth * 0.5),
                Color.Black * 0f,
                Color.Red,
                topOfBar + new Vector2(0f - Game1.dialogueFont.MeasureString("999/999").X - 32f, 64f));
        }
