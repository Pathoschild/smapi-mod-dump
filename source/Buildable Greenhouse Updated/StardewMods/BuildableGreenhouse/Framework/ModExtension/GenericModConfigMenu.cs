...
namespace BuildableGreenhouse.Framework.ModExtension
{
    public static partial class ModExtension
    {
        ...
        private static partial void GenericModConfigMenuExtention()
        {
            ...
            {
                Helper.Events.GameLoop.GameLaunched += buildGMCMConfigMenu;
            }
        }

        private static void buildGMCMConfigMenu(object sender, GameLaunchedEventArgs e)
        {
            KeyValuePair<bool, ExtendedBuildingModel> GreenhouseBuilding = SolidFoundationsApi.GetBuildingModel("BuildableGreenhouse.Greenhouse");

            GenericModConfigMenuApi.Register(
                mod: Manifest,
                reset: () => Config = new ModConfig(),
                save: () => SaveConfig(GreenhouseBuilding.Value)
            );
            ...
            GenericModConfigMenuApi.AddBoolOption(
                ...
                name: () => StartWithGreenhouse_Name,
                tooltip: () => StartWithGreenhouse_Tooltip,
                fieldId: "BuildableGreenhouse.StartWithGreenhouse"
            );
            ...
            GenericModConfigMenuApi.AddNumberOption(
                ...
                name: () => BuildDays_Name,
                tooltip: () => BuildDays_Tooltip,
                fieldId: "BuildableGreenhouse.BuildDays"
            );

            GenericModConfigMenuApi.AddNumberOption(
                mod: Manifest,
                getValue: () => Config.BuildingDifficulty,
                setValue: value => Config.BuildingDifficulty = setBuildingDifficulty(value),
                name: () => BuildingDifficulty_Name,
                tooltip: () => BuildingDifficulty_Tooltip,
                fieldId: "BuildableGreenhouse.BuildingDifficulty"
            );

            GenericModConfigMenuApi.OnFieldChanged(
                mod: Manifest,
                onChange: (string uniqueID, object value) => OnFieldChanged(uniqueID, value, GreenhouseBuilding.Value)
            );
        }

        private static void OnFieldChanged(string uniqueID, object value, ExtendedBuildingModel GreenhouseBuilding)
        {
            if (uniqueID == "BuildableGreenhouse.StartWithGreenhouse")
            {
                if ((bool)value)
            ...
            if (uniqueID == "BuildableGreenhouse.BuildCost")
            {
                GreenhouseBuilding.BuildCost = (int)value;
            }
            if (uniqueID == "BuildableGreenhouse.BuildDays")
            {
                GreenhouseBuilding.BuildDays = (int)value;
            }
            if (uniqueID == "BuildableGreenhouse.BuildingDifficulty")
            {
                GreenhouseBuilding.BuildMaterials = getBuildingMaterial(setBuildingDifficulty((int)value));
            }
        }

        private static void SaveConfig(ExtendedBuildingModel GreenhouseBuilding)
        {
            updateModel(GreenhouseBuilding);
            Helper.WriteConfig(Config);
        }

        private static int setBuildingDifficulty(int value)
        {
            if (value < 0)
                return 0;
            else if (value > 2)
