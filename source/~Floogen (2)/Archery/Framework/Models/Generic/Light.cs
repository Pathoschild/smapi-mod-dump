...
namespace Archery.Framework.Models.ContentPack
{
    public class Light
    {
        ...
        internal bool RecalculateIfElapsed(GameTime time, Random random = null)
        {
            if (random is null)
            {
                random = Game1.random;
            ...
            ElapsedMilliseconds += time.ElapsedGameTime.Milliseconds;
            if (ElapsedMilliseconds >= _updateIntervalMilliseconds)
            {
