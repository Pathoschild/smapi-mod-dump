...
PluginList::FindNodeFromFormat(const char *format) {
	for (auto& plugin : m_plugin_map) {
		const char *the_format = (plugin.second->m_format != NULL) ? plugin.second->m_format : plugin.second->m_plugin->format_proc();

		if (plugin.second->m_enabled) {
...
PluginList::FindNodeFromMime(const char *mime) {
	for (auto &plugin : m_plugin_map) {
		const char *the_mime = (plugin.second->m_plugin->mime_proc != NULL) ? plugin.second->m_plugin->mime_proc() : "";
...
FreeImage_GetFIFFromFilename(const char *filename) {
	...
	for (int i = 0; i < pluginCount; ++i) {
		...
		if (s_plugins->FindNodeFromFIF(i)->m_enabled) {
			...
			auto list = FreeImage_GetFIFExtensionList((FREE_IMAGE_FORMAT)i);

			if (FreeImage_stricmp(list, extension) == 0) {
				...
				memset(copy.get(), 0, len + 1);
				memcpy(copy.get(), list, len);

				// get the first token

				char *token = strtok(copy.get(), ",");
...
FreeImage_GetFIFFromFilenameU(const wchar_t *filename) {
	...
	// get the proper extension if we received a filename
	const wchar_t *place = wcsrchr(filename, '.');	
	if (!place) return FIF_UNKNOWN;
	// convert to single character - no national chars in extensions
	size_t len = wcslen(place);
	auto extension = std::make_unique<char[]>(len + 1);
	for (size_t i = 0; i < len; i++) {
		// convert 16-bit to 8-bit
		extension[i] = (char)(place[i] & 0x00FF);
	}
	extension[len - 1] = '\0';
	FREE_IMAGE_FORMAT fRet = FreeImage_GetFIFFromFilename(extension.get());
