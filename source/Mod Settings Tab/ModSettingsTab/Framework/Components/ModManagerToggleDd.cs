...
{
    public class ModManagerToggleDd : OptionsElement
    {
        ...
        {
            var modPacks = ModManager.PackList.Keys;
            var list = new List<string> {"Default"};
            list.AddRange(modPacks);
            _dropDownBounds.Height = Bounds.Height * list.Count;
            var index = list.FindIndex(s => s == ModManager.ActivePack);
            _dropDownOptions = list;
            _selectedOption = index == -1 ? 0 : index;
        }
        ...
        public override void LeftClickReleased(int x, int y)
        {
            ...
            {
                ModManager.ActivePack = _dropDownOptions[_selectedOption];
            }
            else
