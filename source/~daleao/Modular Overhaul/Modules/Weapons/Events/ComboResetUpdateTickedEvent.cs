...
internal sealed class ComboResetUpdateTickedEvent : UpdateTickedEvent
{
    ...
    protected override void OnEnabled()
    {
        ...
        if (player.CurrentTool is not MeleeWeapon weapon || WeaponsModule.State.ComboHitQueued == ComboHitStep.Idle)
        {
            return;
        }

        WeaponsModule.State.ComboCooldown = (int)(400 * player.GetTotalSwingSpeedModifier(weapon));
    }

    /// <inheritdoc />
    protected override void OnUpdateTickedImpl(object? sender, UpdateTickedEventArgs e)
    {
        WeaponsModule.State.ComboCooldown -= Game1.currentGameTime.ElapsedGameTime.Milliseconds;
        if (WeaponsModule.State.ComboCooldown > 0)
        {
            return;
        }
