...
namespace DaLion.Overhaul.Modules.Weapons.Patchers.Common;
...
internal sealed class InventoryPageDrawPatcher : HarmonyPatcher
{
    ...
    {
        if (WeaponsModule.State.AutoSelectableWeapon is null)
        {
            return;
        }
        ...
        for (var i = 0; i < __instance.inventory.inventory.Count; i++)
        {
            ...
            if (item is Tool tool && WeaponsModule.State.AutoSelectableWeapon == tool)
            {
                component.bounds.DrawBorder(Pixel.Value, 3, WeaponsModule.Config.SelectionBorderColor, b);
            }
        }
    }
