...
internal sealed class InventoryPageDrawPatcher : HarmonyPatcher
{
    ...
    {
        if (SlingshotsModule.State.AutoSelectableSlingshot is null)
        {
            return;
        }
        ...
        for (var i = 0; i < __instance.inventory.inventory.Count; i++)
        {
            ...
            if (item is Tool tool && SlingshotsModule.State.AutoSelectableSlingshot == tool)
            {
                component.bounds.DrawBorder(Pixel.Value, 3, SlingshotsModule.Config.SelectionBorderColor, b);
            }
        }
    }
