...
public static class MineShaftExtensions
{
    ...
    {
        switch (shaft.mineLevel)
        {
            case <= 120 when shaft.mineLevel % 10 == 0:
            case 220 when Game1.player.secretNotesSeen.Contains(10) &&
                          !Game1.player.mailReceived.Contains("qiCave"):
                return true;
            case < 130:
                return false;
            default:
            {
                var isTreasureRoom = Reflector
                    .GetUnboundFieldGetter<MineShaft, NetBool>("netIsTreasureRoom")
                    .Invoke(shaft).Value;
