        ...
        private void SpawnEntity(string command, string[] args) {
            ...
            if (Context.IsWorldReady) {
                ...
                if (args.Length > 0) {
                    ...
                    try { if (args.Length >= 4) { amount = int.Parse(args[3]); if (amount < 1) throw new Exception(); } }
                    ...
                    for (int i = 0; i < amount; i++)
                    {
                        ...
                        else { Monitor.Log($"{args[0]} not found! Type entities to view a list of available entities to spawn!"); return; }
                    }
                    Monitor.Log($"{amount} {entity.Name} added at {entity.currentLocation.Name} {entity.getTileX()},{entity.getTileY()}");               
                }
                else
                {
